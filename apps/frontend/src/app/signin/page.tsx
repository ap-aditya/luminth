...
const getFriendlyFirebaseError = (errorCode: string): { message: string, field?: 'email' | 'password' } => {
    switch (errorCode) {
        ...
        case 'auth/invalid-email':
            return { message: 'The email address format is not valid.', field: 'email' };
        case 'auth/too-many-requests':
            return { message: 'Access to this account has been temporarily disabled due to too many failed attempts. Please try again later.' };
        default:
...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { success, error: authError } = await signInWithEmail(formData.email, formData.password);
    setIsSubmitting(false);
  ...
  if (loading) {
    return <div className="flex justify-center items-center min-h-screen bg-gray-50 dark:bg-slate-900"><p className="text-gray-800 dark:text-gray-200">Loading...</p></div>;
  }
  if (user) {
    return <div className="flex justify-center items-center min-h-screen bg-gray-50 dark:bg-slate-900"><p className="text-gray-800 dark:text-gray-200">Redirecting...</p></div>;
  }
    ...
    <div className="min-h-screen w-full bg-gray-50 dark:bg-slate-950 text-gray-800 dark:text-gray-200 font-sans">
      ...
      <main className="relative z-10 flex flex-col md:flex-row items-center justify-center min-h-screen p-4">
        <div className="w-full max-w-5xl md:grid md:grid-cols-2 rounded-xl shadow-2xl shadow-cyan-500/10 dark:shadow-pink-500/10 overflow-hidden bg-white/50 dark:bg-slate-900/50 backdrop-blur-xl border border-white/10 dark:border-slate-800/50">
          ...
          <div className="w-full p-8 sm:p-12 flex flex-col">
            <div>
              <h1 className="text-3xl font-extrabold mb-2 text-gray-800 dark:text-white">Welcome Back</h1>
              <p className="text-gray-600 dark:text-gray-400">Log in to continue to your dashboard.</p>
            </div>
            ...
            <div>
              <p className="text-center text-gray-600 dark:text-gray-400 text-sm mt-8">
                ...
                <Link href="/signup" className="font-semibold text-cyan-500 hover:text-cyan-400 dark:hover:text-cyan-300 transition-colors">
                  Sign Up
                </Link>
              </p>
            </div>
